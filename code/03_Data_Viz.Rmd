...
df1 <- subset(df, group1 == 1) %>%
  mutate(ame_group1_m_3 = ame_group,
         ame_group1_p_3 = ame_group)

df2 <- subset(df, group2 == 1) %>%
  mutate(ame_group2_m_3 = ame_group,
         ame_group2_p_3 = ame_group)

df3 <- subset(df, group3 == 1) %>%
  mutate(ame_group3_m_3 = ame_group,
         ame_group3_p_3 = ame_group)

df1_dyad <- subset(df_dyad, indiv1 == 1) %>%
  mutate(ame_group1_m_3 = ame_group,
         ame_group1_p_3 = ame_group) %>%
...
df3_dyad <- subset(df_dyad, indiv3 == 1) %>%
  mutate(ame_group3_m_3 = ame_group,
         ame_group3_p_3 = ame_group) %>%
  subset(!is.na(ame_group))

# Weighted mean AME
weighted_mean_ame_group1m <- sum(df1$ame_group1_m_3 * df1$mweight) / sum(df1$mweight)
weighted_mean_ame_group2m <- sum(df2$ame_group2_m_3 * df2$mweight) / sum(df2$mweight)
weighted_mean_ame_group3m <- sum(df3$ame_group3_m_3 * df3$mweight) / sum(df3$mweight)

weighted_mean_ame_group1m_dyad <- sum(df1_dyad$ame_group1_m_3 * df1_dyad$mweight) / sum(df1_dyad$mweight)
weighted_mean_ame_group2m_dyad <- sum(df2_dyad$ame_group2_m_3 * df2_dyad$mweight) / sum(df2_dyad$mweight)
weighted_mean_ame_group3m_dyad <- sum(df3_dyad$ame_group3_m_3 * df3_dyad$mweight) / sum(df3_dyad$mweight)

weighted_mean_ame_group1p <- sum(df1$ame_group1_m_3 * df1$peerweight) / sum(df1$peerweight)
weighted_mean_ame_group2p <- sum(df2$ame_group2_m_3 * df2$peerweight) / sum(df2$peerweight)
weighted_mean_ame_group3p <- sum(df3$ame_group3_m_3 * df3$peerweight) / sum(df3$peerweight)

weighted_mean_ame_group1p_dyad <- sum(df1_dyad$ame_group1_m_3 * df1_dyad$peerweight) / sum(df1_dyad$peerweight)
weighted_mean_ame_group2p_dyad <- sum(df2_dyad$ame_group2_m_3 * df2_dyad$peerweight) / sum(df2_dyad$peerweight)
weighted_mean_ame_group3p_dyad <- sum(df3_dyad$ame_group3_m_3 * df3_dyad$peerweight) / sum(df3_dyad$peerweight)

weighted_var_ame_group1m <- sum(df1$mweight * (df1$ame_group1_m_3 - weighted_mean_ame_group1m)^2) / sum(df1$mweight)
weighted_var_ame_group2m <- sum(df2$mweight * (df2$ame_group2_m_3 - weighted_mean_ame_group2m)^2) / sum(df2$mweight)
weighted_var_ame_group3m <- sum(df3$mweight * (df3$ame_group3_m_3 - weighted_mean_ame_group3m)^2) / sum(df3$mweight)

weighted_var_ame_group1m_dyad <- sum(df1_dyad$mweight * (df1_dyad$ame_group1_m_3 - weighted_mean_ame_group1m_dyad)^2) / sum(df1_dyad$mweight)
...
weighted_var_ame_group3m_dyad <- sum(df3_dyad$mweight * (df3_dyad$ame_group3_m_3 - weighted_mean_ame_group3m_dyad)^2) / sum(df3_dyad$mweight)

weighted_var_ame_group1p <- sum(df1$peerweight * (df1$ame_group1_m_3 - weighted_mean_ame_group1p)^2) / sum(df1$peerweight)
weighted_var_ame_group2p <- sum(df2$peerweight * (df2$ame_group2_m_3 - weighted_mean_ame_group2p)^2) / sum(df2$peerweight)
weighted_var_ame_group3p <- sum(df3$peerweight * (df3$ame_group3_m_3 - weighted_mean_ame_group3p)^2) / sum(df3$peerweight)

weighted_var_ame_group1p_dyad <- sum(df1_dyad$peerweight * (df1_dyad$ame_group1_m_3 - weighted_mean_ame_group1p_dyad)^2) / sum(df1_dyad$peerweight)
...
weighted_var_ame_group3p_dyad <- sum(df3_dyad$peerweight * (df3_dyad$ame_group3_m_3 - weighted_mean_ame_group3p_dyad)^2) / sum(df3_dyad$peerweight)

weighted_se_ame_group1m <- sqrt(weighted_var_ame_group1m / sum(df1$mweight))
weighted_se_ame_group2m <- sqrt(weighted_var_ame_group2m / sum(df2$mweight))
weighted_se_ame_group3m <- sqrt(weighted_var_ame_group3m / sum(df3$mweight))

weighted_se_ame_group1m_dyad <- sqrt(weighted_var_ame_group1m_dyad / sum(df1_dyad$mweight))
weighted_se_ame_group2m_dyad <- sqrt(weighted_var_ame_group2m_dyad / sum(df2_dyad$mweight))
weighted_se_ame_group3m_dyad <- sqrt(weighted_var_ame_group3m_dyad / sum(df3_dyad$mweight))

weighted_se_ame_group1p <- sqrt(weighted_var_ame_group1p / sum(df1$peerweight))
weighted_se_ame_group2p <- sqrt(weighted_var_ame_group2p / sum(df2$peerweight))
weighted_se_ame_group3p <- sqrt(weighted_var_ame_group3p / sum(df3$peerweight))

weighted_se_ame_group1p_dyad <- sqrt(weighted_var_ame_group1p_dyad / sum(df1_dyad$peerweight))
weighted_se_ame_group2p_dyad <- sqrt(weighted_var_ame_group2p_dyad / sum(df2_dyad$peerweight))
weighted_se_ame_group3p_dyad <- sqrt(weighted_var_ame_group3p_dyad / sum(df3_dyad$peerweight))
...
plot_data_ame <- data.frame(
  group = c("group1_m", "group2_m", "group3_m", "group1_p", "group2_p", "group3_p"),
  weight = c("m", "m", "m", "p", "p", "p"),
  mean = c(weighted_mean_ame_group1m, weighted_mean_ame_group2m, weighted_mean_ame_group3m,
           weighted_mean_ame_group1p, weighted_mean_ame_group2p, weighted_mean_ame_group3p),
  se = c(weighted_se_ame_group1m, weighted_se_ame_group2m, weighted_se_ame_group3m,
         weighted_se_ame_group1p, weighted_se_ame_group2p, weighted_se_ame_group3p)
)

plot_data_ame_dyad <- data.frame(
  group = c("group1_m", "group2_m", "group3_m", "group1_p", "group2_p", "group3_p"),
  weight = c("m", "m", "m", "p", "p", "p"),
  mean = c(weighted_mean_ame_group1m_dyad, weighted_mean_ame_group2m_dyad, weighted_mean_ame_group3m_dyad,
           weighted_mean_ame_group1p_dyad, weighted_mean_ame_group2p_dyad, weighted_mean_ame_group3p_dyad),
  se = c(weighted_se_ame_group1m_dyad, weighted_se_ame_group2m_dyad, weighted_se_ame_group3m_dyad,
         weighted_se_ame_group1p_dyad, weighted_se_ame_group2p_dyad, weighted_se_ame_group3p_dyad)
)
...
plot_data_ame %>%
  ...
  labs(x = "Variables", y = "Predicted Margins", title = "") +
  #geom_hline(yintercept = 0, linetype = "dotted", color = "grey") + 
  ylim(-0.085,0.025) +
  annotate("text", y = c(weighted_mean_ame_group1m,
                         weighted_mean_ame_group2m,
                         weighted_mean_ame_group3m), 
           x = c(2.7,1.7,0.7),
...
plot_data_ame_dyad %>%
  ...
  #geom_hline(yintercept = 0, linetype = "dotted", color = "grey") + 
  ylim(-0.057,0.025) +
  annotate("text", y = c(weighted_mean_ame_group1m_dyad,
                         weighted_mean_ame_group2m_dyad,
                         weighted_mean_ame_group3m_dyad), 
           x = c(2.7,1.7,0.7),
           label = c("Anti-immigration", "Moderate", "Pro-immigration"),
           color = colors[1:3],
           size = 3) +
