...
Table_1_model_w <- read_rds(here("results", "Table_1_model.rds"))

#setup individual plotting

df <- df %>%
  mutate(index1 = ifelse(index == 2, NA, index),
         index2 = ifelse(index != 2, NA, index))
...
Table_1_model_w[3:4, 2:5] <- lapply(Table_1_model_w[3:4, 2:5], function(x) {
...
f1a <- ggplot() +
  ...
  }, Table_1_model_w[1:4,2], x_positions, y_pos1) +
  mapply(function(txt, x, y) {
    annotate("text", x = x, y = y, label = txt, color = "#453781FF")}, 
    Table_1_model_w[1:4,3], x_positions, y_pos4) +
  mapply(function(txt, x, y) {
    annotate("text", x = x, y = y, label = txt, color = "#2D708EFF")}, 
    Table_1_model_w[1:4,4], x_positions, y_pos3) +
  mapply(function(txt, x, y) {
    annotate("text", x = x, y = y, label = txt, color = "#3CBB75FF")}, 
    Table_1_model_w[1:4,5], x_positions, y_pos2) +
...
f1b <- df %>%
  ...
  scale_color_manual(values = c("#453781FF", "#3CBB75FF")) +   # Color for index1 groups
  scale_fill_manual(values = c("#2D708EFF")) +  
  ...
  annotate("text", x = -0.45, y = 0.49, label = "Anti-immigration\nteams", hjust = 0, color = "#453781FF", size = 2.7) +
  geom_segment(x = -0.4, y = 0.2, xend = -0.4, yend = 0.12,
               arrow = arrow(type = "closed", length = unit(0.07, "inches"), angle = 15),
               color = "#453781FF", size = 0.09) +
    geom_segment(x = -0.225, y = 0.26, xend = -0.195, yend = 0.26,
               arrow = arrow(type = "closed", length = unit(0.07, "inches"), angle = 15),
               color = "#453781FF", size = 0.09) +
  annotate("text", x = 0.42, y = 0.49, label = "Pro-immigration\nteams", hjust = 1, color = "#3CBB75FF", size = 2.7) +
  geom_segment(x = 0.37, y = 0.2, xend = 0.31, yend = 0.1,
               arrow = arrow(type = "closed", length = unit(0.07, "inches"), angle = 15),
               color = "#3CBB75FF", size = 0.09) +
  theme_classic() +
    geom_segment(x = 0.23, y = 0.23, xend = 0.17, yend = 0.18,
               arrow = arrow(type = "closed", length = unit(0.07, "inches"), angle = 15),
               color = "#3CBB75FF", size = 0.09, linetype = "longdash") +
    annotate("text", x = -0.01, y = 0.25, label = "Moderate\nteams", color = "#2D708EFF", size = 2.7) +
  theme_classic() +
