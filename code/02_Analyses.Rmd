...
tbl_5_model <- df %>% 
  select(index, Scale, Stock, Flow, level_cyear, allavailable, w1996, w2006, w2016)
```
...
df_junkrej <- df_junkrej %>%
  mutate(h_sup = ifelse(Hresult == "Support", 1, 0),
         h_nosup = ifelse(Hresult == "Reject" | Hresult == "No test", 1, 0),
         h_rej = ifelse(Hresult == "Reject", 1, 0),
         rate_categories = case_when(
           pos_test_pct_p05 < 0.075 ~ 1,
           pos_test_pct_p05 < 0.325 ~ 2,
           pos_test_pct_p05 < 0.5 ~ 3,
           TRUE ~ 4),
...
e_tot <- lm(h_sup ~ proindex + pb1 + stats_brw + topic_brw + team_size + mdegree, data = df_junkrej, weights = inv_weight_R)

e_tot_r <- lm(h_rej ~ proindex + pb1 + stats_brw + topic_brw + team_size + mdegree, data = df_junkrej, weights = inv_weight_R)

e_dir <- lm(h_sup ~ meaname + proindex + pb1 +  stats_brw + topic_brw + team_size + mdegree, data = df_junkrej, weights = inv_weight_R)

e_dir_both <- lm(h_sup ~ pos_test_pct_p05 + neg_test_pct_p05 + proindex + pb1 +  stats_brw + topic_brw + team_size + mdegree, data = df_junkrej, weights = inv_weight_R)

e_dir_r <- lm(h_rej ~ meaname + proindex + pb1 +  stats_brw + topic_brw + team_size + mdegree, data = df_junkrej, weights = inv_weight_R)

e_ind <- lm(meaname ~ proindex + pb1 + stats_brw + topic_brw + team_size + mdegree, data = df_junkrej, weights = inv_weight_R)

e_ind_both <- lm(pos_test_pct_p05 ~ proindex + pb1 + stats_brw + neg_test_pct_p05 + topic_brw + team_size + mdegree, data = df_junkrej, weights = inv_weight_R)

e_ind_r <- lm(meaname ~ proindex + pb1 + stats_brw + topic_brw + team_size + mdegree, data = df_junkrej, weights = inv_weight_R)

e_ind2 <- lm(pb1 ~ proindex + stats_brw + topic_brw + team_size + mdegree, data = df_junkrej, weights = inv_weight_R)
...
a <- coef(e_ind)["proindex"]  # Coefficient of proindex in the indirect model
b <- coef(e_dir)["meaname"]  # Coefficient of pos_test_pct_p05 in the direct model

# Step 2: Extract the standard errors
se_a <- summary(e_ind)$coefficients["proindex", "Std. Error"]  # Standard error for proindex in e_ind
se_b <- summary(e_dir)$coefficients["meaname", "Std. Error"]  # Standard error for pos_test_pct_p05 in e_dir
...
```{r esobn}
a <- coef(e_ind_r)["proindex"]  # Coefficient of proindex in the indirect model
b <- coef(e_dir_r)["meaname"]  # Coefficient of pos_test_pct_p05 in the direct model

# Step 2: Extract the standard errors
se_a <- summary(e_ind_r)$coefficients["proindex", "Std. Error"]  # Standard error for proindex in e_ind
se_b <- summary(e_dir_r)$coefficients["meaname", "Std. Error"]  # Standard error for pos_test_pct_p05 in e_dir
...
Table_Effects[1,2:7] <- c("H Supported", "H Supported", "Mean AME", "H Rejected", "H Rejected", "Mean AME")

Table_Effects[,1] <- c("DV =", "Mean AME", "", "Pro-Immigrant", "", "Belief in H", "", "Stat Skills", "", "Topic Experience", "")
