...
df2 <- df2 %>%
  mutate(pscore_Z = (pscore - mean(pscore)) / sd(pscore)) # standardize
```
...
df3 <- df3 %>%
  mutate(indiv2 = ifelse(indiv == 1, 1, 0),
         indiv1 = ifelse(indiv == 2, 1, 0),
         indiv3 = ifelse(indiv == 3, 1, 0)) %>%
  group_by(u_id) %>%
  mutate(nmodel = n()) %>%
  ungroup() %>%
  subset(!is.na(indiv) & !is.na(AME))

df3$pweight <- 1/(df_dyad$nmodel*df_dyad$team_size)
