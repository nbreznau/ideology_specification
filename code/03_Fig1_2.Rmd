...
agg_png(here("results", "Fig1S.png"), res = 144, width = 600, height = 500)
...
f1b <- df_dyad %>%
  ...
  geom_density(data = df_dyad %>% filter(!is.na(indiv2) & indiv2 == 1),
               aes(x = AME), color = NA, fill = colors[2], 
               alpha = 0.3, adjust = 5.5, position = "identity") +
  
  # Second density for indiv1, excluding NA values
  geom_density(data = df_dyad %>% filter(!is.na(indiv1) & indiv2 != 1),
               aes(x = AME, group = indiv, color = factor(indiv)), 
               alpha = 1, adjust = 5, position = "identity") +  
  # Labels
...
knitr::include_graphics(here("results", "Fig1S.png"))
```
