...
### Fig5A
```{r f4arrange}
agg_png(here("results", "Fig5.png"), res = 144, height = 700, width = 900)
...
### Fig 5B. Plot AME distribution

```{r 5b}
agg_png(here("results", "Fig5b.png"), res = 144, height = 650, width = 850)

ggplot() +
  stat_density(data = df, aes(x = ame_combo_mean, group = index, color = factor(index)),
               adjust = 5, position = "identity", size = 1, geom = "line", trim = TRUE) +
  labs(x = "Average AME of unique model specifications",
    ...
    annotate("label", x = -0.25,
           y = 7,  # Adjust `y` to position text in the lower-left
           label = "Anti-immigration teams\nchose model specifications\nthat overwhelmingly produced\nnegative results on average",
...
agg_png(here("results", "Fig5C.png"), res = 144, height = 1000, width = 850)
...
ggplot() +
  ...
  annotate("text", x = 2.5, y = -1.2, label = "Unique combinations of eight\nkey modelling decisions", angle = 90, color = "black", size = 4.5, hjust = 0.5, vjust = 0.5) +
  annotate("text", x = 3.5, y = -0.65, label = "=  +0.020*", size = 3.8, color = colors[3]) +
  annotate("text", x = 3.5, y = -1.75, label = "=  -0.021*", size = 3.8, color = colors[1]) +
  ...
  annotate("text", x = 1.4, y = -2.2, label = "Anti-immigration", size = 3.8, color = colors[1], hjust = 0) +
    geom_point(data = df_points2, aes(x = x, y = y, fill = fill, color = fill), size = 5, shape = 21) +
      annotate("label", x = 3.35,
           y = -1.2,  
           label = "The resulting unique\nspecifications explain\nroughly half of\nthe ideology effect",
           color = "black",  # Set text color to colors[1]
           fill = "white",     # Background fill color of the box
           label.size = 0.5,   # Border thickness
           label.r = unit(0.25, "lines"),  # Rounded corners
