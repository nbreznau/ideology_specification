#delimit ;
log using /volumes/ddisk/mi/researcher/reg_teamlevel.log, replace;
clear matrix;
clear mata;
...
**************************************************;
*THIS PROGRAM AGGREATES TEAM LEVEL DATA TO ONE OBSERVATION PER TEAM;
...
collapse (sum) nmodel negsig possig (mean) meaname=ame varame, by(teamid);
...
*GET TEAM LEVEL DATA;
*BUT WILL ALL ALSO COLLAPSE TO 71 TEAMS;
...
*COLLAPSING TEAM DATA TO ONE OBSERVATION PER TEAM;
collapse (mean) ame  proindex index p12 p56 stats_brw topic_brw mdegree pbelief belief_brw
	pmale belief_1_avg  team_size hresult hreject hrej hsupport hnotest, 
	by(teamid);
...
sort teamid;
merge teamid using /volumes/ddisk/data/junkvar.dta;
drop _merge;

drop if ame==.;

*NOTE THAT HREJ IS NOT CONSTANT WITHIN A TEAM;
*IT HAS TO DO WITH THE 16 TEAMS THAT PROVIDED CONFLICTING CONCLUSIONS;
*SO HREJECT IS A MUCH BETTER VARIABLE TO USE IN ANALYSIS;
tabulate hrej hreject;

generate group1=(index==1);
generate group2=(index==2);
generate group3=(index==3);

*REPLICATING BASIC REGRESSIONS AT TEAM LEVEL;
regress meaname  proindex stats_brw topic_brw i.team_size i.mdegree  , cluster(teamid);
regress meaname  group3 stats_brw topic_brw i.team_size i.mdegree  , cluster(teamid);
regress meaname group1 group3 stats_brw topic_brw i.team_size i.mdegree  , cluster(teamid);



summ ame meaname;
*NOTE THAT THE MEAN AME FROM THE COLLAPSE OF THE 87 OBSERVATIONS TO 71 OBSERVATIONS;
*DOES NOT EXACTLY REPLICATE THE TRUE MEANAME FROM THE MODELS ESTIMATED BY THAT TEAM;


*REPLICATING BASIC REGRESSIONS AT TEAM LEVEL;
regress ame  proindex stats_brw topic_brw i.team_size i.mdegree  , cluster(teamid);
regress ame  group3 stats_brw topic_brw i.team_size i.mdegree  , cluster(teamid);
regress ame  group1 group3 stats_brw topic_brw i.team_size i.mdegree  , cluster(teamid);
...
***************************************************************;
*REPLICATING BASIC REGRESSIONS AT 87-TEAM-HREJ LEVEL;
...
collapse (sum) nmodel negsig possig (mean) meaname=ame varame, by(teamid hrej);
generate rpos=possig/nmodel;
generate rneg=negsig/nmodel;
...
*THE MEAN AME FOR EACH OF THE 87 TEAMS IS EXACTLY EQUAL TO WHAT YOU GET BY COLLAPSING CRI.DTA INTO 87 TEAMS;
summ meaname ame;
